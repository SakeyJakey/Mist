...
public class ConfigPanel extends Draggable {
	...
	MainClickGui cgui;
	private final ConcurrentHashMap<String, Boolean> configs = new ConcurrentHashMap<String, Boolean>();
	...
	public void draw() {
		...
		for (String c :
			...
			if (Mist.instance.getConfigManager().isCurrentlySelected(c)) // todo add is config loaded add
				RenderUtils.drawRect(x, y, w, h + (configs.get(c) ? settingSize * fr.getFontHeight() : 0), ColourUtil.dim(ColourUtil.getRainbow(4, count * 50L), 1));
